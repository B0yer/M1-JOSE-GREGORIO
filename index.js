...
app.post('/api/registrar-cita', async (req, res) => {
    ...
    try {
        ...
        // 🔔 NOTIFICACIÓN a todos los admin/superusuario
        // Se usa 'admin' como usuario_destino fijo: el GET filtra por este valor
...
app.put('/api/admin/actualizar-reparacion', async (req, res) => {
    ...
    try {
        // Consultar estado actual ANTES de actualizar para detectar transición real de aceptación
        const { data: citaAntes } = await supabase
            .from('citas')
            .select('cedula, estado')
            .eq('id', id)
            .single();
        const estadoAnterior = citaAntes?.estado || '';
        ...
        const citaData  = citaAntes; // ya lo tenemos
        const { data: adminData } = await supabase.from('usuarios').select('nombre, apellido, cedula').eq('cedula', id_admin).single();
        const { data: clienteData } = citaData?.cedula
            ? await supabase.from('usuarios').select('nombre, apellido').eq('cedula', citaData.cedula).single()
            : { data: null };
        ...
        // Registrar 'Cita Aceptada' SOLO si es la primera vez (venía de Pendiente o Confirmada)
        const esTransicionAceptacion = (nuevoEstado || 'Aceptada') === 'Aceptada' &&
            ['Pendiente', 'Confirmada'].includes(estadoAnterior);
        if (esTransicionAceptacion) {
            await registrarAuditoria({
                usuario_afectado: cedCliente ? `${nomCliente} (${cedCliente})` : null,
                accion: 'Cita Aceptada',
                detalles: `${nomAdmin} aceptó la cita #${id} del cliente ${nomCliente}.`,
                responsable: nomAdmin,
                tipo: 'cita'
            });
        }
...
app.get('/api/admin/auditoria', async (req, res) => {
    ...
    try {
        ...
        // Filtros por acción específica (subtipos de cita)
        const FILTROS_ACCION = {
            'cita_aceptada':        'Cita Aceptada',
            'cita_rechazada':        'Cita Rechazada',
            'cita_finalizada':       'Cita Finalizada',
            'cita_entregada':        'Vehículo entregado',
            'reparacion_actualizada':'Reparación actualizada'
        };

        if (tipo && tipo !== 'todos' && tipo !== 'sistema' && !FILTROS_ACCION[tipo]) {
            query = tipo === 'cita'
                ? query.eq('tipo', 'cita')
                : query.eq('tipo', tipo);
        }
        ...
        } else if (FILTROS_ACCION[tipo]) {
            // Filtro por acción específica dentro de las citas
            resultado = data.filter(item =>
                item.accion === FILTROS_ACCION[tipo]
            );
        } else if (tipo === 'cita') {
            resultado = data.filter(item =>
...
// ==========================================
// 24. ENTREGA DE VEHÍCULO — CREA NOTIFICACIÓN CON PDF EMBEBIDO
// ==========================================
// POST /api/admin/entregar-vehiculo
// Body: { id, cedula_cliente, placa, pdf_base64, fecha_actualizacion }
app.post('/api/admin/entregar-vehiculo', async (req, res) => {
    const { id, cedula_cliente, placa, pdf_base64, fecha_actualizacion, id_admin } = req.body;

    if (!id || !cedula_cliente) {
        return res.status(400).json({ success: false, message: 'id y cedula_cliente son requeridos.' });
    }

    try {
        // 1. Actualizar estado de la cita a 'Entregado'
        ...
        if (errorEstado) {
            console.warn('⚠️ No se pudo actualizar estado a Entregado:', errorEstado.message);
        }

        // 2. Crear notificación con el PDF en base64 embebido
        const mensaje = `🚗 Tu vehículo (${placa || 'Placa N/A'}) ha sido entregado. ¡Descarga tu constancia desde el historial!`;

        const { error: errorNotif } = await supabase.from('notificaciones').insert([{
            usuario_destino: String(cedula_cliente),
            mensaje,
            leido: false,
            pdf_base64: pdf_base64 || null   // columna nueva en la tabla notificaciones
        }]);

        if (errorNotif) {
            // Si la columna pdf_base64 no existe aún, insertamos sin ella
            console.warn('⚠️ Error al insertar con pdf_base64, reintentando sin PDF:', errorNotif.message);
            await supabase.from('notificaciones').insert([{
                usuario_destino: String(cedula_cliente),
                mensaje,
                leido: false
        ...
        // 3. Obtener nombre del cliente y del admin para auditoría
        const { data: clienteData } = await supabase
            .from('usuarios')
            .select('nombre, apellido')
            ...
            .single();
        const nomCliente = clienteData
            ? `${clienteData.nombre} ${clienteData.apellido} (${cedula_cliente})`
            : cedula_cliente;

        // Si el frontend no envió id_admin, lo tomamos de la cita (guardado al aceptar)
        let cedAdmin = id_admin;
        if (!cedAdmin) {
            const { data: citaRow } = await supabase
                ...
                .eq('id', id)
                .single();
            cedAdmin = citaRow?.id_admin || null;
        }

        let nomAdmin = 'Admin desconocido';
        if (cedAdmin) {
            const { data: adminData } = await supabase
                ...
                .single();
            if (adminData) {
                nomAdmin = `${adminData.nombre} ${adminData.apellido} (${adminData.cedula})`;
            }
        }

        // 4. Auditoría
        await registrarAuditoria({
            usuario_afectado: nomCliente,
            accion: 'Vehículo entregado',
            detalles: `${nomAdmin} registró la entrega del vehículo placa ${placa} (cita #${id}) al cliente ${nomCliente}. Constancia PDF generada.`,
            responsable: nomAdmin,
            tipo: 'cita'
        });

        console.log(`✅ Vehículo entregado. Cita #${id}, cliente ${cedula_cliente}`);
        res.json({ success: true, message: 'Entrega registrada y notificación enviada al cliente.' });

    } catch (err) {
        console.error('💥 Error en entregar-vehiculo:', err);
        res.status(500).json({ success: false, message: 'Error interno del servidor.' });
    }
});
